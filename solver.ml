  ...
  struct
    ...
    let solve (g:Graph.t) : Graph.t =
      let worklist = Graph.nodes g in
			let rec repeat nodes g : Graph.t =
				if Graph.NodeS.is_empty nodes
          then g
				  else
            let n = Graph.NodeS.choose nodes in
					  let rm = Graph.NodeS.remove n nodes in
					  let new_out = Graph.flow g n (Fact.combine (List.map (fun pred -> Graph.out g pred) (Graph.NodeS.elements (Graph.preds g n)))) in
            let g' = Graph.add_fact n new_out g in
            repeat (if (Fact.compare (Graph.out g n) new_out) <> 0 then
            Graph.NodeS.union rm (Graph.succs g' n)
            else rm) g'
      in
			repeat worklist g
  end
