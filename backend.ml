...
module LocSetMap = Map.Make (struct type t = Alloc.loc let compare = (fun x y ->
  let s1 = str_locset (LocSet.singleton x) in
  let s2 = str_locset (LocSet.singleton y) in
...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  let rec colorgraph regs spills adjlst mappedcolors =

  if (Datastructures.UidM.is_empty adjlst) then
    mappedcolors, spills, regs
  else
	let vertices_deg_lt_k = Datastructures.UidM.filter (fun _ v -> UidSet.cardinal v < 7) adjlst in
  if (Datastructures.UidM.is_empty vertices_deg_lt_k) then
    let (v, n) = Datastructures.UidM.choose adjlst in
    let (spilled, vertices) = Datastructures.UidM.fold (fun d v (d', v') ->
      if (UidSet.cardinal v') < (UidSet.cardinal v) then
      (d, v) else (d', v')) adjlst (v, n) in
    colorgraph regs (1 + (fst spills), [(spilled, Alloc.LStk (- (1 + (fst spills))))] @ (snd spills))
    (UidSet.fold (fun neighbour set ->
      Datastructures.UidM.add neighbour (UidSet.remove spilled (Datastructures.UidM.find neighbour adjlst)) set)
      vertices (Datastructures.UidM.remove spilled adjlst))
    (Datastructures.UidM.add spilled (Alloc.LStk (- (1 + (fst spills)))) mappedcolors)
  else
  let (v, n) = Datastructures.UidM.choose vertices_deg_lt_k in
  let v', neighbours = Datastructures.UidM.fold (fun d v (d', v') ->
    if (UidSet.cardinal v') < (UidSet.cardinal v) then
    (d, v) else (d', v')) vertices_deg_lt_k (v, n) in

  let (g'_colored, spilled', regs') =
    colorgraph regs spills
    (UidSet.fold (fun neighbour set ->
      Datastructures.UidM.add neighbour (UidSet.remove v' (Datastructures.UidM.find neighbour adjlst)) set)
      neighbours (Datastructures.UidM.remove v' adjlst))
    mappedcolors in

  let col = LocSet.choose (UidSet.fold (fun adj_node acc -> LocSet.remove (Datastructures.UidM.find adj_node g'_colored) acc)
  neighbours (LocSet.(remove (Alloc.LReg Rcx) (remove (Alloc.LReg Rax) caller_save)))) in
  (Datastructures.UidM.add v' col g'_colored, spilled', (LocSetMap.add col ((LocSetMap.find col regs') + 1) regs')) in

  let blks = [("", (fst f.f_cfg))] @ (snd f.f_cfg) in

  let n_arg = ref 0 in
  let n_spill = ref 0 in

  let alloc_arg = List.map (fun x -> (x, (let res =
    begin match arg_loc !n_arg with
      | Alloc.LReg Rcx -> (incr n_spill; Alloc.LStk (- !n_spill))
      | _ -> arg_loc !n_arg
    end
  in
  incr n_arg; res))) f.f_param in

	let rig = Datastructures.UidM.fold (fun v neighbours set ->
    Datastructures.UidM.add v (UidSet.remove v neighbours) set)
    (List.fold_left
      (fun a (lbl, blk) ->
        let live' = live (fst blk.term) in
        UidSet.fold (fun uid lst ->
        if Datastructures.UidM.mem uid lst
        then Datastructures.UidM.add uid (UidSet.union (Datastructures.UidM.find uid lst) (live')) lst
        else Datastructures.UidM.add uid (live') lst)
    (live')
    (List.fold_left
    (fun a' uid ->
        let live'' = live uid in
        UidSet.fold
        (fun uid a'' ->
          if Datastructures.UidM.mem uid a''
          then Datastructures.UidM.add uid (UidSet.union (Datastructures.UidM.find uid a'') (live'')) a''
          else Datastructures.UidM.add uid (live'') a'')
        (live'') (if Datastructures.UidM.mem uid a' then a' else Datastructures.UidM.add uid UidSet.empty a'))
    a (List.map fst blk.insns))) (Datastructures.UidM.empty) blks) Datastructures.UidM.empty in

  let (rig', colors) =
    List.fold_left (fun (rig, mapped_colors) (l, blk) ->
      List.fold_left (fun (a, a') (i, ins)->
        if insn_assigns ins then (a, a')
        else
        (Datastructures.UidM.remove i (UidSet.fold (fun n a ->
        Datastructures.UidM.add n (UidSet.remove i (Datastructures.UidM.find n a)) a)
        (Datastructures.UidM.find i a) a), Datastructures.UidM.add i Alloc.LVoid a'))
        (rig, mapped_colors) blk.insns)
        (rig, Datastructures.UidM.empty) blks in

  let (g_colored, spills, _) =
    colorgraph (List.fold_left (fun a r -> LocSetMap.add r 0 a) LocSetMap.empty (List.map (fun reg -> Alloc.LReg reg) [ Rdi; Rsi; Rdx; R09; R08; R10; R11 ]))
    (!n_spill, []) rig' colors in

  { uid_loc = (fun id ->
    if Datastructures.UidM.mem id g_colored then Datastructures.UidM.find id g_colored
    else if List.exists (fun x -> x = id) f.f_param then List.assoc id alloc_arg
    else if List.exists (fun x -> id = (fst x)) (snd f.f_cfg) then List.assoc id (List.map (fun (lbl, _) -> (lbl, Alloc.LLbl (Platform.mangle lbl))) (snd f.f_cfg))
    else failwith "error")
